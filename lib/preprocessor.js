...
function preprocessOas(oass, options) {
  ...
  oass.forEach(oas => {
    ...
    data.options.report.numOpsMutation += Oas3Tools.countOperationsMutation(
      oas
    );
    ...
    const commonSecurityPropertyName = utils_1.getCommonPropertyNames(
      data.security,
      currentSecurity
    );
 ...
 */
function mergeAllOfAndDereference(schema, oas) {
  return mergeAllOf(
    schema.allOf.map(entry => {
      if (entry.$ref) {
        entry = Oas3Tools.resolveRef(entry.$ref, oas);
      }
      if ("allOf" in entry) {
        entry = mergeAllOfAndDereference(entry, oas);
      }
      return entry;
    })
  );
...
function createDataDef(names, schema, isInputObjectType, data, links, oas) {
  ...
  if ("allOf" in schema) {
    schema = mergeAllOfAndDereference(schema, oas);
  ...
  if (index !== -1) {
    ...
    if (typeof saneLinks !== "undefined") {
      if (typeof existingDataDef.links !== "undefined") {
         ...
         */
        existingDataDef.links = Object.assign(
          {},
          saneLinks,
          existingDataDef.links
        );
      } else {
      ...
      if (type === "array" && typeof schema.items === "object") {
        ...
        if ("$ref" in itemsSchema) {
          if (oas) {
            ...
            if ("allOf" in itemsSchema) {
              itemsSchema = mergeAllOfAndDereference(itemsSchema, oas);
            }
          } else {
